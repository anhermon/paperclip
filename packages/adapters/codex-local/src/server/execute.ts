...
export async function execute(ctx: AdapterExecutionContext): Promise<AdapterExecutionResult> {
  ...
  const ensureSkipGitRepoCheck = (args: string[]): string[] => {
    if (args.includes("--skip-git-repo-check")) return args;
    if (args.length > 0 && args[args.length - 1] === "-") {
      return [...args.slice(0, -1), "--skip-git-repo-check", "-"];
    }
  ...
  const runAttempt = async (resumeSessionId: string | null) => {
    ...
    const args = ensureSkipGitRepoCheck(execArgs.args);
