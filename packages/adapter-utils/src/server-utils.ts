...
export async function listPaperclipSkillEntries(
  ...
  try {
    ...
    return Promise.all(dirs.map(async (entry) => {
      ...
      const isCoreSkill = entry.name === "paperclip";
      // Core skill is always required; others respect `required: false` in SKILL.md frontmatter.
      const required = isCoreSkill || await readSkillRequired(skillDir);
      return {
        key: `paperclipai/paperclip/${entry.name}`,
        runtimeName: entry.name,
        source: skillDir,
        required,
        requiredReason: required ? "Core Paperclip skill required by all agents." : null,
        roles,
      };
    }));
  } catch {
